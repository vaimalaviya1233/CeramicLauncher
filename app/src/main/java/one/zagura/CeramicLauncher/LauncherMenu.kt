...
import one.zagura.CeramicLauncher.tools.open
        ...
        if (!isActive) {
            ...
            dialog = Dialog(home, R.style.longpressmenusheet).apply {
                ...
                findViewById<View>(R.id.custombtn).run {
                    setOnClickListener {
                        home.open(CustomMain::class.java, ActivityOptions.makeCustomAnimation(home, R.anim.slideup, R.anim.home_exit).toBundle())
                        dialog?.dismiss()
                    }
                }
